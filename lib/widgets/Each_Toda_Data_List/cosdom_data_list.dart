...
class COSDOMDriversDataList extends StatefulWidget {
  const COSDOMDriversDataList({super.key});

  @override
  State<COSDOMDriversDataList> createState() => _COSDOMDriversDataListState();
}

class _COSDOMDriversDataListState extends State<COSDOMDriversDataList> {
  ...
  void initState() {
    ...
    if (currentUser != null) {
      _listenForBlockStatus(currentUser.uid);
    }
  }
  ...
  void _listenForBlockStatus(String driverId) {
    _userStatusRef = FirebaseDatabase.instance.ref().child("drivers").child(driverId).child("blockStatus");

    // Listen to the changes in block status
    _statusSubscription = _userStatusRef.onValue.listen((event) async {
      String? blockStatus = event.snapshot.value as String?;

      if (blockStatus == "yes") {
        // If the block status is "yes", log out the user
        await _auth.signOut();
